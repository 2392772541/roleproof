...
import { buildMatches, calculateScore } from './lib/engine'
...
function App() {
  ...
  const [jobs, setJobs] = useLocalStorage<Job[]>('roleproof.jobs.v1', demoJobs)
  const [evidence, setEvidence] = useLocalStorage<Evidence[]>('roleproof.evidence.v1', demoEvidence)
  const [selectedJobId, setSelectedJobId] = useLocalStorage('roleproof.selected-job.v1', demoJobs[0].id)
  const [decisions, setDecisions] = useLocalStorage<DecisionLog[]>('roleproof.decisions.v1', [])
  const [portfolio, setPortfolio] = useLocalStorage<PortfolioSection[]>('roleproof.portfolio.v1', [])
  const [interviews, setInterviews] = useLocalStorage<InterviewRecord[]>('roleproof.interviews.v1', [])
  const fileRef = useRef<HTMLInputElement>(null)

  const selectedJob = jobs.find((job) => job.id === selectedJobId) ?? jobs[0]
  const matches = useMemo(() => buildMatches(selectedJob, evidence), [selectedJob, evidence])
  ...
  const recordDecision = (targetType: DecisionLog['targetType'], targetId: string, status: DecisionStatus, note = '') => {
    setDecisions((items) => [
      { id: `D-${Date.now()}`, targetType, targetId, status, note, createdAt: new Date().toISOString() },
      ...items.filter((item) => !(item.targetType === targetType && item.targetId === targetId)),
    ])
  }

  const exportWorkspace = () => {
    const blob = new Blob([JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), jobs, evidence, decisions, portfolio, interviews }, null, 2)], { type: 'application/json' })
  ...
  const importWorkspace = (file?: File) => {
    ...
    reader.onload = () => {
      try {
        const data = JSON.parse(String(reader.result)) as { jobs?: Job[]; evidence?: Evidence[]; decisions?: DecisionLog[]; portfolio?: PortfolioSection[]; interviews?: InterviewRecord[] }
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${menuOpen ? 'is-open' : ''}`}>
      ...
      <div className="sidebar-footer"><div className="progress-ring"><span>82%</span></div><span><strong>证据档案完整度</strong><small>6 个项目 · 18 份材料</small></span></div>
    </aside>
    ...
    <main>
      <header className="topbar">
        ...
        <div className="topbar-actions">
          <label className="job-switcher"><span>{selectedJob.company}</span><select value={selectedJob.id} onChange={(event) => setSelectedJobId(event.target.value)}>{jobs.map((job) => <option key={job.id} value={job.id}>{job.company} · {job.title}</option>)}</select><ChevronDown size={15} /></label>
          <button className="icon-button" title="导入" onClick={() => fileRef.current?.click()}><Upload size={18} /></button><input ref={fileRef} hidden type="file" accept="application/json" onChange={(event) => importWorkspace(event.target.files?.[0])} />
      ...
      <div className="content">
        {view === 'dashboard' && <Dashboard jobs={jobs} evidence={evidence} decisions={decisions} interviews={interviews} chooseJob={chooseJob} navigate={navigate} />}
        {view === 'intelligence' && <Intelligence jobs={jobs} chooseJob={chooseJob} onCreate={(job) => setJobs((items) => [job, ...items])} />}
        ...
        {view === 'interview' && <InterviewReview job={selectedJob} evidence={evidence} interviews={interviews} setInterviews={setInterviews} recordDecision={recordDecision} />}
      </div>
    </main>
  </div>
}
