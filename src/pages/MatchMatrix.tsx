...
export function MatchMatrix({ job, evidence, decisions, recordDecision, navigate }: Props) {
  ...
  return <>
    ...
    <div className="score-layout"><section className="panel score-card"><div className="score-gauge" style={{ '--score': `${score.total * 3.6}deg` } as React.CSSProperties}><div><strong>{score.total}</strong><span>/ 100</span></div></div><div><span className="status-badge success"><i />规则评分</span><h2>证据匹配度</h2><p>五项规则子分相加得到总分，不能代替实际录用概率。</p></div></section><section className="panel score-breakdown">{breakdown.map(([label, value, max]) => <div key={label}><span>{label}</span><i><b style={{ width: `${value / max * 100}%` }} /></i><strong>{value}<small> / {max}</small></strong></div>)}</section></div>
